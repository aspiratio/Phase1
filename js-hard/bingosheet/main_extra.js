...
//1~5行目（0~4列目,斜め）のヒットした数字を数えるための配列を作る
const horizontal = []; //horizontal[インデックス番号（行番号）] 横
const vertical = []; //縦
let diagonal1 = 0; //左上から右下
let diagonal2 = 0; //左下から右上
...
function searchCell(num) {
	...
	if (lineNum !== -1) { //上記でnumを含む配列が見つかれば
		...
		//三項演算子 オブジェクトの中身があればvertical[lineNum]+1を、なければ（undefinedなら）1をvertical[lineNum]に代入する
		horizontal[lineNum] = horizontal[lineNum] ? horizontal[lineNum] + 1 : 1;
		vertical[columnNum] = vertical[columnNum] ? vertical[columnNum] + 1 : 1;
		
		for (i = 0; i < 5; i++) {
			if (lineNum === i + 1 && columnNum === i) { //斜めラインのインデックス番号が出たら＋1
				diagonal1++;
			} else if (lineNum === i + 1 && columnNum === 4 - i) {
				diagonal2++;
			}
		}
	}
...
//ヒットした数字が5つ並んでいたらビンゴと表示する処理
function searchBingo() {
	const judgeBingo = horizontal.includes(5) || vertical.includes(5) || diagonal1 === 5 || diagonal2 === 5;
	const judgeReach = horizontal.includes(4) || vertical.includes(4) || diagonal1 === 4 || diagonal2 === 4;
	if (judgeBingo) {
		alert("ビンゴ！");
		reach.textContent = "ビンゴ！";
	} else if (judgeReach) {
		reach.textContent = "リーチ";
	}
}
...
const numbers = document.getElementById("numbers"); //当選番号表示場所
...
hitNum.addEventListener("click", function() {
	if (nonSelectedNum.length !== 0) {
		...
		const hitted = document.createElement("li"); //リストタグの作成
		hitted.textContent = selectedNum; //リストタグにテキスト追加
		numbers.prepend(hitted); //ノード（リストタグ）をnumbersの子ノードの先頭に追加
